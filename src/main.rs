...
async fn run() {
    let numbers = if std::env::args().len() <= 1 {
        ...
        std::env::args()
            .skip(1)
            .map(|s| f32::from_str(&s).expect("You must pass a list of floats!"))
            .collect()
    };
...
async fn execute_gpu(numbers: Vec<f32>) -> Vec<f32> {
    ...
    let shader_src = include_str!("shader.comp");
    let shader_spirv = shader_compiler.compile_into_spirv(shader_src, shaderc::ShaderKind::Compute, "shader.comp", "main", None).unwrap();
    let shader_data = wgpu::util::make_spirv(shader_spirv.as_binary_u8());
